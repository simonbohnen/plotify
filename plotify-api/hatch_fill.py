...
# hatch_fill.py
#
# Based upon eggbot_hatch.py from the Eggbot project / evilmadscientist,
...
# Updated by Maren Hachmann, 2021-02-04
# Updated extension for Inkscape 1.0.2, to the degree that it does not produce deprecation warnings.
# Added option to choose a unit for the spacing and inset.
# Extension is now independent of other EggBot extensions.
# Clones didn't work properly before, ask user to unlink.
# There are probably lots of functions that can now be replaced by inkex functionality.
# 
...
class Hatch_Fill(inkex.Effect):
    ...
    def handleViewBox(self):
        ...
        self.docTransform = Transform(f"scale({self.svg.scale},{self.svg.scale}")

    def addPathVertices(self, node):
        ...
        if node.tag in [
            ...
            # We use a copy for the shapes, don't want to convert everything to a path
            node_as_path = node.to_path_element()
            p = node_as_path.path
        else:
            p = node.path

        # Get a cubic super duper path  
        p = p.transform(transform).to_superpath()
    ...
    def joinFillsWithNode(self, node, stroke_width, path):
        ...
        try:
            ...
            hatch = etree.SubElement(g, inkex.addNS('path', 'svg'), line_attribs)
            hatch.transform = inverse_parent_transform
    ...
    def effect(self):
        ...
        self.options.hatchSpacing = self.svg.unittouu(str(self.options.hatchSpacing) + self.options.unit)
        self.options.holdBackSteps = self.svg.unittouu(str(self.options.holdBackSteps) + self.options.unit)
        ...
        # stroke_width_target = ( self.svg.height + self.svg.width ) / 2000
        ...
        for key in self.hatches:
            ...
            if key in self.transforms:
                transform = -Transform(self.transforms[key])
